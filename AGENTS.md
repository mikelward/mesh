...
- Before starting any task, run `git fetch origin main`. When worktrees are
  available, create a fresh worktree on a fresh branch based on the latest
  `origin/main` with `git worktree add -b <branch> <path> origin/main`;
  otherwise create a fresh branch from it. If continuing an existing branch,
  rebase it onto the latest `origin/main` before the first new commit, resolving
  any conflicts rather than working from an older base.
