...
/// ([`crate::parser::tokenize`]) rather than a bespoke char scan, so this cannot
...
fn signature_close(after_open: &str) -> Option<usize> {
    ...
    let tokens = crate::parser::tokenize(after_open).ok()?;
...
mod tests {
    ...
    fn needs_more_input_skips_comments_when_scanning_a_signature() {
        ...
        assert!(needs_more_input("func f(x = 1 + # note )\n"));
        ...
        assert!(!needs_more_input("func f(x = /#tag) { puts $x }"));
        assert!(!needs_more_input("func f(x = /#tag) oops\n"));
        ...
        assert!(needs_more_input("func f(x = /#tag)\n"));
    }

    #[test]
    fn needs_more_input_dispatches_a_reserved_or_duplicate_final_name() {
        ...
        assert!(needs_more_input("func f(a\n"));
        assert!(needs_more_input("func f(a, b\n"));
        assert!(needs_more_input("func f(ab"));
    ...
    #[test]
    fn needs_more_input_dispatches_an_impossible_parameter_ordering() {
        ...
        assert!(!needs_more_input("func f(a = 1, b\n"));
        assert!(!needs_more_input("func f(...xs, a\n"));
        assert!(!needs_more_input("func f(a = 1, ...xs\n"));
        assert!(!needs_more_input("func f(...xs,\n"));
        ...
        assert!(needs_more_input("func f(a = 1, b"));
        assert!(needs_more_input("func f(a = 1, b = 2\n"));
        assert!(needs_more_input("func f(--tag = latest, a\n"));
        assert!(needs_more_input("func f(a, ...xs\n"));
        ...
        // keeps buffering.
        assert!(!needs_more_input("func f(a\n= 1\n"));
        assert!(!needs_more_input("func f(--flag\n= 1\n"));
        assert!(needs_more_input("func f(a =\n1\n"));
    ...
    #[test]
    fn needs_more_input_dispatches_a_mismatched_signature_delimiter() {
        ...
        assert!(needs_more_input("func f(x = $(puts hi))\n"));
        assert!(!needs_more_input("func f(x = $(puts hi)) { puts $x }"));
    ...
    fn needs_more_input_buffers_a_block_bearing_default() {
        ...
        assert!(!needs_more_input("func f(x = if true { 1 ])\n"));
        assert!(!needs_more_input("func f(x = { 1 ])\n"));
    }
