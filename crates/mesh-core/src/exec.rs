...
fn signal_number(name: &str) -> Option<libc::c_int> {
    if let Ok(number) = name.parse::<libc::c_int>() {
        ...
        // which signals this platform has: a fixed ceiling here rejected valid
        // ones (Linux's `SIGRTMAX` is 64) and would need a per-platform table to
        // avoid it. An impossible number comes back as `EINVAL`, reported
        // against the target the way any other failure to signal is.
        //
        // Zero is a signal here: `kill -0` sends nothing and reports whether the
        // target exists and could be signalled, which is how a script asks
