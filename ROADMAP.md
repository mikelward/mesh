...
- **The history list** — Up or Down shows the five most recent commands
  containing the typed line beneath it, then the ones matching it fuzzily,
  the way a browser's address bar does, with the first already on the line
  so Up, Enter still runs the last one.
