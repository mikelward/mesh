...
- **Requiring `else` where an `if` yields a value — reopened; no lean, and the
  ...
  **The real obstacle is that "value position" is not a clean category in mesh.**
  ...
  | **Fail on the gap** | The only option that is loud *at the moment the omission bites*, and it needs no new principle — `main`'s own value clause already says it for every other operation: *an operation that hands back a value fails when it has none*. It is also the only row that needs **no boundary rule**: the check is dynamic, so "value position" never has to be defined syntactically, which is precisely the obstacle stalling the two `else` rows above. Nothing is rejected at write time, so the terse one-liner stays writable | It converts the [prompt-segment idiom](INTRO.md) from working to **failing on the branch it is designed to skip**, so that idiom needs its `else { "" }` after all — the same casualty as the broad `else` rule, arriving at run time instead of parse time. And that is the general trade: this fires per *execution* where the `else` rows fire per *program*, so a gap in a rarely-taken branch sits unnoticed until it runs, which is the worse direction for a config file read constantly and exercised occasionally. It also cannot tell "the author forgot" from "the author meant nothing", so unlike the `else` rows it leaves the soft channel no terse spelling at all — every deliberate nothing must be written `else { "" }`. Statement position is unaffected, having no value to be missing |


  **Recorded, not decided — and not blocking anything.** mikelward's lean is
  toward requiring the total branch (the second row), with the consequences
  still being weighed; the sixth row was added because the five above it weigh
  only *where a branch may be omitted* and *what the gap yields*, and none of
  them weighs failing on it, which is what the value clause implies. Today's
  behavior stands meanwhile: a lone `if` and an unmatched `match` both yield
  `""`, verified. The `match` half of this question is weighed separately under
  `TODO.md` §"Decisions needed", *How loud should a mismatch be?* — that entry's
  six answers all assume today's `""`, so whichever way this lands re-frames
